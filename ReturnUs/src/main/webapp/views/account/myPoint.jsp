										...
										<div class="row">
											...
											<table class="table">
											  ...
											  <tbody>
											      ...
											      <c:choose>
														<c:when test="${coll.colUsePnt ne 0 && coll.colTotalPnt ne 0 }">
														  <tr>
														  	<td>${coll.colGetpntDt }</td>
											      			<td>${coll.colNum }</td>																			      
													        <td style="color:blue">+<fmt:formatNumber type="number" pattern="###,###,###,###,###,###" value="${coll.colTotalPnt }" /></td>
													      </tr>													  
														  <tr>
														  	<td>${coll.colDate }</td>	
											      			<td>${coll.colNum }</td>																		      
														...
														<c:when test="${coll.colUsePnt eq 0 && coll.colTotalPnt ne 0 }">
														  <tr>
														  	<td>${coll.colGetpntDt }</td>
											      			<td>${coll.colNum }</td>																			      
													        <td style="color:blue">+<fmt:formatNumber type="number" pattern="###,###,###,###,###,###" value="${coll.colTotalPnt }" /></td>
														...
														<c:when test="${coll.colUsePnt ne 0 && coll.colTotalPnt eq 0 }">
														  <tr>
														  	<td>${coll.colDate }</td>	
											      			<td>${coll.colNum }</td>																		      
													        <td style="color:red">-<fmt:formatNumber type="number" pattern="###,###,###,###,###,###" value="${coll.colUsePnt }" /></td>
										...
										<div id="pageDiv" class="col-12">
		                                     <div class="pagination d-flex justify-content-center mt-5">
		                                     	<c:choose>
													...
													<c:otherwise>
														<a href="my-point?accId=${accId }&page=${pageInfo.curPage-1 }" class="rounded">&lt;</a>
													</c:otherwise>
												</c:choose>
												<c:forEach begin="${pageInfo.startPage }" end="${pageInfo.endPage }" var="i">
													<c:choose>
														<c:when test="${i == pageInfo.curPage }">
															<a href="my-point?accId=${accId }&page=${i }" class="rounded" style="background: #81c408;color: white;">${i}</a>
														</c:when>
														<c:otherwise>
															<a href="my-point?accId=${accId }&page=${i }" class="rounded">${i}</a>
														</c:otherwise>
													</c:choose>
												</c:forEach>
												<c:choose>
